...
import type { ChatMessage, GameState } from '@/types'; // Added GameState
...
interface ChatPanelProps {
  isModalMode?: boolean;
  userStatus: GameState['status']; // Use GameState['status'] for clarity
}

const ChatPanel: FC<ChatPanelProps> = ({ isModalMode = false, userStatus }) => {
  const { user } = useAuth();
  const { gameState } = useGameLogic(); // Get gameState for displayName
    ...
    const unsubscribe = onValue(messagesRef, (snapshot) => {
      ...
      if (data) {
        const loadedMessages = Object.keys(data).map(key => ({
          id: key,
          ...data[key]
        } as ChatMessage)); // Assert type for loaded messages
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    const messageData: Omit<ChatMessage, 'id'> = { // Omit id as it's generated by push
      senderUid: user.uid,
      senderDisplayName: senderDisplayName,
      text: newMessage.trim(),
      timestamp: serverTimestamp() as any, // Cast to any for serverTimestamp placeholder
    };
          ...
          <div className="space-y-3">
            {messages.map((msg) => (
              <div key={msg.id} className="text-sm leading-relaxed">
                <span className={`font-semibold ${msg.senderUid === user?.uid ? 'text-accent' : 'text-primary/90'}`}>
                  {msg.senderDisplayName || 'Người lạ'}:
                </span>{' '}
