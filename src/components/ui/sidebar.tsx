...
const SIDEBAR_WIDTH = "16rem" // Default 256px
const SIDEBAR_WIDTH_MOBILE = "18rem" // For mobile sheet
const SIDEBAR_WIDTH_ICON = "3.5rem" // Increased for slightly larger icons if needed (56px)
...
>(
  (
    ...
    React.useEffect(() => {
        if (isMobile) {
            setOpen(false); // Always collapse on desktop if mobile view is active
        }
    }, [isMobile, setOpen]);
      ...
      >
        ...
        <div
          className={cn(
            ...
            // Adjust padding and borders for variants
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-8 w-8 md:h-7 md:w-7", className)} // Adjusted size
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      {...props}
    >
      <PanelLeft className="h-5 w-5" /> {/* Adjusted icon size */}
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-3 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-px hover:after:bg-sidebar-border group-data-[side=left]:-right-3 group-data-[side=right]:left-0 sm:flex",
        "[[data-side=left]_&]:cursor-w-resize [[data-side=right]_&]:cursor-e-resize",
        !open && "[[data-side=left]_&]:cursor-e-resize",
        !open && "[[data-side=right]_&]:cursor-w-resize",
  ...
  return (
    <div
      ...
      className={cn(
        ...
        (state === 'expanded' && "md:peer-data-[variant=floating]:ml-[calc(var(--sidebar-width)_+_theme(spacing.2))]"),
        (state === 'collapsed' && "md:peer-data-[variant=floating]:ml-[calc(var(--sidebar-width-icon)_+_theme(spacing.2)_+_theme(spacing.2))]"),
...
>(({ className, ...props }, ref) => {
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "h-8 w-full bg-sidebar-primary/10 text-sidebar-foreground shadow-none focus-visible:ring-2 focus-visible:ring-sidebar-ring placeholder:text-sidebar-foreground/60",
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-1 p-2 overflow-y-auto overflow-x-hidden",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="group"
      className={cn("relative flex w-full min-w-0 flex-col", className)} // Removed p-2 for more granular control
      {...props}
    />
  )
})
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "group-data-[collapsible=icon]:mx-auto group-data-[collapsible=icon]:mt-2 group-data-[collapsible=icon]:h-auto group-data-[collapsible=icon]:px-0 group-data-[collapsible=icon]:text-[10px]", // Icon only style
        className
      )}
      {...props}
    />
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute right-3 top-1.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-none ring-sidebar-ring transition-transform hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2.5 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-colors duration-150 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-primary data-[active=true]:font-medium data-[active=true]:text-sidebar-primary-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:!size-9 group-data-[collapsible=icon]:!p-0 [&>span:last-child]:truncate group-data-[collapsible=icon]:[&>span:last-child]:hidden [&_svg]:size-5 group-data-[collapsible=icon]:[&_svg]:size-5 [&_svg]:shrink-0",
  {
    variants: {
      ...
      size: { // Size variants for text part, icon size fixed in base
        default: "h-9 text-sm", // Approx 36px
        sm: "h-8 text-xs",    // Approx 32px
        lg: "h-10 text-sm",   // Approx 40px
      },
    },
    defaultVariants: {
      variant: "default",
...
const SidebarMenuButton = React.forwardRef<
  HTMLButtonElement, // Changed to HTMLButtonElement for better semantics if not `asChild`
    ...
    return (
      <Tooltip>
        ...
        <TooltipContent
          side="right"
          align="center"
          // Show tooltip only when sidebar is collapsed (icon-only) and not on mobile
...
>(({ className, asChild = false, showOnHover = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute right-1.5 top-1/2 -translate-y-1/2 flex aspect-square w-6 h-6 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-none ring-sidebar-ring transition-opacity hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 peer-hover/menu-button:text-sidebar-accent-foreground [&>svg]:size-4 [&>svg]:shrink-0",
        "after:absolute after:-inset-1 after:md:hidden", // Increased hit area on mobile
        "group-data-[collapsible=icon]:hidden",
        showOnHover &&
...
const SidebarMenuBadge = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> // Changed to HTMLAttributes
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
...
>(({ className, showIcon = true, ...props }, ref) => { // Default showIcon to true
  const width = React.useMemo(() => {
    return `${Math.floor(Math.random() * 30) + 60}%` // Width between 60-90%
  }, [])
  ...
  return (
    <div
      ...
      className={cn("rounded-md h-9 flex gap-2.5 px-2 items-center", className)} // Matched SidebarMenuButton style
      {...props}
    >
      {showIcon && (
        <Skeleton
...
>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn(
      "ml-[calc(theme(spacing.2)_+_theme(spacing[0.5]))] mr-2 flex min-w-0 translate-x-px flex-col gap-0.5 border-l border-sidebar-border pl-[calc(theme(spacing.2)_+_theme(spacing[0.5]))] py-1",
      "group-data-[collapsible=icon]:hidden",
      className
    )}
    {...props}
...
>(({ asChild = false, size = "default", isActive, className, children, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex min-w-0 items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground/80 outline-none ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-1 focus-visible:ring-sidebar-ring active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0 [&_svg]:text-sidebar-foreground/60",
        "data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground data-[active=true]:font-medium data-[active=true]:[&_svg]:text-sidebar-accent-foreground",
        size === "sm" && "h-7 text-xs",
