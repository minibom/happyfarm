...
export const usePlotActions = ({
  ...
  const plantCrop = useCallback((plotId: number, seedId: SeedId) => {
    ...
    if (analytics) {
      logEvent(analytics, 'plant_crop', {
        item_id: seedId,
        item_name: cropDetail.name,
        plot_id: plotId,
      });
    }
  ...
  const harvestCrop = useCallback(async (plotId: number) => {
    ...
    if (analytics) {
      logEvent(analytics, 'harvest_crop', {
        item_id: plotToHarvest.cropId,
        item_name: cropDetail.name,
        quantity: cropDetail.harvestYield,
        plot_id: plotId,
      });
      if (earnedXp > 0) {
  ...
  const unlockPlot = useCallback((plotIdToUnlock: number) => {
    ...
    if (analytics) {
      logEvent(analytics, 'unlock_plot', {
        plot_id: plotIdToUnlock,
        cost: cost,
      });
      logEvent(analytics, 'spend_virtual_currency', {
        value: cost,
        virtual_currency_name: 'gold',
        item_name: `unlock_plot_${plotIdToUnlock}`,
      });
    }
  ...
  const applyFertilizer = useCallback((plotId: number, fertilizerId: FertilizerId) => {
    ...
    if (analytics) {
      logEvent(analytics, 'apply_fertilizer', {
        item_id: fertilizerId,
        item_name: fertilizerDetail.name,
        plot_id: plotId,
        crop_id: plot.cropId,
      });
    }
  ...
  const uprootCrop = useCallback((plotId: number) => {
    ...
    const uprootedCropId = plotToUproot.cropId;
    ...
    if (analytics && uprootedCropId && cropData && cropData[uprootedCropId]) {
      logEvent(analytics, 'uproot_crop', {
        item_id: uprootedCropId,
        item_name: cropData[uprootedCropId].name,
        plot_id: plotId,
      });
    }
  }, [setGameState, gameStateRef, toast, cropData]);
